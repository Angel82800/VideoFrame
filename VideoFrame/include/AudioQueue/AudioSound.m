...
- (id)initWithSoundFile:(NSURL*)filename
{
    if (!(self = [super init])) return nil;
    [self loadSoundFile:filename];
    return self;
}
- (void)loadSoundFile:(NSURL*)filename
{
//    NSString *soundFile= [[[NSBundle mainBundle] resourcePath] stringByAppendingPathComponent:filename];
    CFURLRef soundURL = CFURLCreateWithFileSystemPath(kCFAllocatorDefault, (CFStringRef)[filename absoluteString], kCFURLPOSIXPathStyle, false);
