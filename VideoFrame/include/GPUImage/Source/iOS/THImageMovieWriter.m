...
- (void)startRecording;
{
    dispatch_group_notify([THImageMovieManager shared].readingAllReadyDispatchGroup, dispatch_get_main_queue(), ^{
        ...
        if (self.assetAudioReader != nil)   //added by Yinjing Li at 2015/06/08
        {
            BOOL aduioReaderStartSuccess = [self.assetAudioReader startReading];
            
            if(_hasAudioTrack && !aduioReaderStartSuccess)
            {
...
- (void)createDataFBO;
{
    ...
    if ([GPUImageContext supportsFastTextureUpload])
    {
        ...
        CVBufferSetAttachment(renderTarget, kCVImageBufferYCbCrMatrixKey, kCVImageBufferYCbCrMatrix_ITU_R_709_2, kCVAttachmentMode_ShouldPropagate);
